...
test('AddBadge', async t => {
  ...
  t.truthy(userBadge[0].date)
  t.deepEqual(userBadge[0].name, badge, 'user must be have an avatar')
...
test('addSkill', async t => {
  ...
  t.deepEqual(userSkills[0], skill)
...
test('addPoints', async t => {
  ...
  t.deepEqual(userPoints, 8)
  t.throws(db.addPoints(dude), /not found/)
})

test('addImage', async t => {
  let db = t.context.db
  t.is(typeof db.addImage, 'function', 'addPoints to user function')
  ...
  let userImages = await db.addImage(user.username)

  t.deepEqual(userImages, 1)
  t.throws(db.addImage(dude), /not found/)
})
