...
class Database {
  ...
  addSkill (username, skill, cb) {
    ...
    let tasks = co.wrap(function* () {
      ...
      if (!utils.findSkill(skill)) {
        return Promise.reject(new Error('invalid skill'))
      }
  ...
  createPicture (image, cb) {
    ...
    try {
      image.userId = uuid.decode(image.userId)
    } catch (e) {
      return Promise.resolve({error: 'invalid user id, non base62'})
    }

    let tasks = co.wrap(function * () {
      let conn = yield connection

      let userDb = yield r.db(db).table('users').get(image.userId).run(conn)
  ...
  getPicture (imageId, cb) {
    ...
    imageId = uuid.decode(imageId)

    let tasks = co.wrap(function * () {
      let conn = yield connection

      let imageDb = yield r.db(db).table('images').get(imageId).run(conn)

      if (!imageDb) {
        return Promise.reject(new Error(`image: ${uuid.encode(imageId)} not found`))
      }
  ...
  deletePicture (imageId, cb) {
    ...
    let tasks = co.wrap(function * () {
      ...
      let response = yield r.db(db).table('images').get(imageId).delete().run(conn)
  ...
  addPictureAward (imageId, award, cb) {
    if (!this.connected) {
      return Promise.reject(new Error('Not connected')).asCallback(cb)
    }
    ...
    let tasks = co.wrap(function* () {
      ...
      let pictureDb = yield getPicture(imageId)
      ...
      let sponsorName = sponsor.username

      if (_.includes(pictureDb.sponsors, sponsorName)) {
        return Promise.reject(new Error('this account has already given an award'))
      }

      let awards = pictureDb.awards
      awards[award.type] = awards[award.type] + 1

      yield r.db(db).table('images').get(pictureDb.id).update({
        awards: awards,
        sponsors: r.row('sponsors').append(sponsorName)
      }).run(conn)
