...
class Database {
  ...
  connect (cb) {
    ...
    const setup = co.wrap(function * () {
      ...
      if (dbTables.indexOf('contributions') === -1) {
        ...
        yield r.db(db).table('contributions').indexCreate('comunityRate').run(conn)
        yield r.db(db).table('contributions').indexCreate('devApproval').run(conn)
      }
  ...
  createUser (user, cb) {
    ...
    */
    ...
    const tasks = co.wrap(function * () {
      ...
      if (user.username === process.env.MUTAR_ADMIN_NAME) {
        user.admin = true
      }
  ...
  createContrib (contribution, username, cb) {
    ...
    */
    ...
    if (!contribution.title || !contribution.data) {
      return Promise.reject(new Error('Invalid contribution'))
    }
    ...
    let tasks = co.wrap(function * () {
      ...
      // date create
      contribution.dateAdded = new Date()
      ...
      contribution.data.type = contribution.data.type | 'message'

      // Community
      contribution.messages = []
      contribution.comunityRate = 0
      ...
      // getting tags
      let message = contribution.data.info
      let tags = utils.getTags(message)

      contribution.tags = tags

      let result = yield r.db(db).table('contributions').insert(contribution).run(conn)
  ...
  modifyContrib (contributionId, contributionChanges, user, cb) {}
  rateContrib (contributionId, scoringUser, cb) {}
  evaluateModeContrib (contributionId, userKey, cb) {}
  addMessageContrib (contributionId, message, user, cb) {}

  // admin actions
  createDev (user) {
    // como se va a crear un dev?
    // podria darse un routepoint
  }
  ...
  devRes (contributionId, user, devResponse, cb) {
    ...
    let tasks = co.wrap(function * () {
      ...
      let status = 200
      let message = devResponse.message
      // debe activar la respuesta del dev
      // debe asignar la respuesta a la contribucion

      // debe crearse una respuesta
      let res = {
        status: status,
        message: message
      }
