  ...
  createUser (user, cb) {
    ...
    */
    ...
    const tasks = co.wrap(function * () {
      ...
      user.forename = user.forename || 'nobody'
  ...
  createContrib (contribution, username, cb) {
    ...
    */
    ...
    let tasks = co.wrap(function * () {
      ...
      yield r.db(db).table('contributions').get(id).update({
        publicId: uuid.encode(id)
      }).run(conn)

      let contributionCreated = yield r.db(db).table('contributions').get(id).run(conn)
